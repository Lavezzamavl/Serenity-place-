...
DISCHARGE_ROLE_KEYWORDS = ('nurse', 'doctor', 'physician', 'psychiatrist')


class CanDischarge(BasePermission):
    def has_permission(self, request, view):
        user = request.user
        if not user or not user.is_authenticated:
            return False
        ...
        return any(keyword in name for keyword in DISCHARGE_ROLE_KEYWORDS)
