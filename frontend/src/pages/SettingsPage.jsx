...
export default function SettingsPage() {
  ...
  useEffect(() => { getFacilitySettings().then(setForm).finally(() => setLoading(false)); }, []);
  useEffect(() => { getAuditLogs().then(setLogs).catch(() => {}).finally(() => setLogsLoading(false)); }, []);
  ...
  if (loading) return <div className="flex items-center justify-center py-16 text-slate"><Loader2 className="w-5 h-5 animate-spin mr-2" /> Loading settings...</div>;

  return (
    <div className="max-w-lg">
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center gap-2 mb-5"><SettingsIcon className="w-4 h-4 text-serenity" /><h4 className="font-medium text-harbor">Facility Settings</h4></div>
        <form onSubmit={handleSave} className="space-y-3">
          ...
          <div>
            ...
            <input type="number" value={form.rehab_package_price} onChange={(e) => setForm({ ...form, rehab_package_price: e.target.value })}
              className="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-serenity" />
          </div>
