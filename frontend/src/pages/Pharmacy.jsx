...
import { Pill, AlertTriangle, Search, Plus, X } from 'lucide-react';
import { getDrugs, addStock } from '../api/pharmacy';
...
export default function Pharmacy({ user }) {
  ...
  useEffect(() => {
    loadDrugs().finally(() => setLoading(false));
  }, []);
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-16 text-slate">Loading pharmacy stock...</div>;
  }
