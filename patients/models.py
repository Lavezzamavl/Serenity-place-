...
class Patient(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Keep discharged_at in sync with status regardless of entry point
        # (API, admin, shell) - stamp it the moment status flips to
        # Discharged, clear it if the patient is re-admitted.
        if self.status == 'Discharged' and self.discharged_at is None:
            self.discharged_at = timezone.now()
        elif self.status == 'Admitted' and self.discharged_at is not None:
